...
class EpisodeReplayBufferSkillLearn(EpisodeReplayBuffer):
    ...
    def _advance(self):
        self._top = (
            self._top + self.n_envs * self.max_path_length
    ...
    def add_path(self, path):
        ...
        self._low_level_float_observations[
            self._top : self._top + self.n_envs * self.max_path_length
        ] = path["low_level_float_observations"]
        self._high_level_actions[
            self._top : self._top + self.n_envs * self.max_path_length
    ...
    def get_diagnostics(self):
        ...
        diagnostics["average_returns"] = (
            self._low_level_rewards[: self._size].sum(axis=1).mean()
        )
        diagnostics["average_final reward"] = self._low_level_rewards[: self._size][
            :, -1
        ].mean()
        return diagnostics
